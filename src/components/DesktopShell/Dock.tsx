...
type Props = { counts: Record<Kind, number>; onDockClick: (t: Kind) => void }
...
export default function Dock({ counts, onDockClick }: Props) {
  ...
  // ---- tuning (your values) ----
  const SIGMA           = 50       // spread of influence (px)
  const VICINITY_BAND   = 140      // px above dock where it activates
  const HOVER_SCALE     = 1.06     // scale when pointer is over an icon
  const NEAR_SCALE      = 1.06     // scale for neighbors when near but not over
  const LIFT_HOVER      = 10       // px up on hover
  const LIFT_NEAR       = 3        // px up when near but not over
  const PUSH_NEAR_TOWARD= 6        // px nudge TOWARD cursor when near (subtle)
  ...
  useEffect(() => {
    const onMove = (e: PointerEvent | MouseEvent) => { mouse.current = { x: e.clientX, y: e.clientY } }
  ...
  const inVicinity = (pt: Pt) => {
    ...
    return pt.y >= topBand && pt.y <= bottomBand &&
           pt.x >= box.left - 60 && pt.x <= box.right + 60
  }
  ...
  const computeTargets = () => {
    ...
    items.forEach(({ id }) => {
      ...
      if (active && btn && pt) {
        ...
        if (isOver(pt, btn)) {
          // pointer directly over icon → lift + expand only
          t.s = HOVER_SCALE
          t.ty = -LIFT_HOVER
        } else {
          // near but not over → subtle tilt away & nudge toward cursor
          const strength = Math.exp(-(dist*dist)/(2*SIGMA*SIGMA))
          t.s   = 1 + (NEAR_SCALE - 1) * strength
          t.ty  = -LIFT_NEAR * strength
          t.tx  = -(dir !== 0 ? dir : 0) * PUSH_NEAR_TOWARD * strength
          t.rot =  (dir !== 0 ? dir : 0) * TILT_DEG * strength
        }
      }
      tgtRef.current[id] = t
    })
      ...
      <div ref={wrapRef}
        ...
        <div className="flex items-end justify-center gap-8">
          {items.map((it) => {
            ...
            return (
              <div key={it.id}
                   ref={(el)=>{ itemWrapRefs.current[`dock-wrap-${it.id}`] = el }}
